...
import { generateSignature, getTotalPrice } from "@/lib/utils";
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { paymentMethod, voucherCode, deliveryFee } = data;
    const product = await Product.findById(data.orderItems[0].productID);
    if (!product) {
    ...
    const orderItems = [
      { ...data.orderItems[0], productID: product, selected: true },
    ];
