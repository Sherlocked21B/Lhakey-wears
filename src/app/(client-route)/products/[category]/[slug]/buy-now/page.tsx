...
import { buyNowOrder, createNewOrder } from "@/services/order";
...
export default function BuyNowPage() {
  ...
  const subTotal =
    cartItems.length < 1
      ? 0
      : cartItems[0].productID.onSale === "yes"
      ? finalPrice(
          cartItems[0].productID.price,
          cartItems[0].productID.discountPercentage as number
        ) * cartItems[0].quantity
      : cartItems[0].productID.price * cartItems[0].quantity;

  const totalPayment = subTotal + (isInsideValley ? 0 : 150);
  ...
  const handlePayment = async () => {
    ...
    if (
      ...
      toast.error("Please add your name in your profile");
    ...
    const res = await buyNowOrder(formData);
    if (res.success) {
      if (res.data) {
  ...
  useEffect(() => {
    setCartItems(
      localStorage.getItem("cartDetail")
        ? [JSON.parse(localStorage.getItem("cartDetail") as string)]
        : []
    );
  }, [user]);
  ...
  return (
    <div className="flex h-full divide-x-3 my-container py-32">
